...
const GridRow: FC<GridRowProps> = ({ column, style, rowIndex, align, item, i }) => {
  const { classes, cx } = useStyles();
  // const { name, address, imageUrl } = item.asset;
  // const { t } = useTranslation('assets');
  ...
  switch (column) {
    ...
    case 'price':
      ...
      break;
    case 'supply':
      formatItem = <div className={classes.supply}>{supply}</div>;
      break;
    case 'market_cap':
      formatItem = <div className={classes.marketCap}>$ {marketCap}</div>;
      break;
    case 'holders':
      formatItem = <div className={classes.holders}>{holders}</div>;
      break;
    case 'last7Days':
      formatItem = `chart`;
      break;
